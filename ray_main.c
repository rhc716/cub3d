...
void	init_window(t_ray *ray, t_env *env)
{
	...
	ray->img->data = (int *)mlx_get_data_addr(ray->img->img, &ray->img->bpp
	, &ray->img->size_l, &ray->img->endian);
}
...
int		main_loop(t_env *env)
{
	...
	if (env->map_mode == 1)
	{
		...
		draw_map(env);
		draw_lines(env, ray->tilesize_x, ray->tilesize_y);
	...
	ray_loop(env, ray);
	draw_sprite(env, ray);
	mlx_put_image_to_window(env->ray->mlx, env->ray->win
	, env->ray->img->img, 0, 0);
...
int		ray_caster(t_env *env, t_err *err, int argc)
{
	...
	|| !(ray->sp_env = (t_sp_env *)malloc(sizeof(t_sp_env))))
